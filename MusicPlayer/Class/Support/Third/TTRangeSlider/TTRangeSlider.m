...
@property (nonatomic, strong) CALayer *sliderLineOnLeftHandle;
...
- (void)initialiseControl {
    ...
    self.sliderLineOnLeftHandle = [CALayer layer];
    self.sliderLineOnLeftHandle.backgroundColor = self.tintColor.CGColor;
    [self.layer addSublayer:self.sliderLineOnLeftHandle];
...
#pragma mark   fqyMark_barSidePadding数值不能动
...
- (void)updateHandlePositions {
    ...
    self.sliderLineOnLeftHandle.frame = CGRectMake(self.frame.origin.x, self.sliderLine.frame.origin.y, self.leftHandle.frame.origin.x-self.frame.origin.x, self.lineHeight);
...
- (void)setDisableRightHandle:(BOOL)disableRightHandle {
    _disableRightHandle = disableRightHandle;
    if (_disableRightHandle) {
...
- (void)setSelectedMinimum:(float)selectedMinimum {
    ...
    }else if (selectedMinimum > self.selectedMaximum){
        selectedMinimum = self.selectedMaximum;
    }
...
- (void)setTintColorLineOnLeftHandle:(UIColor *)tintColorLineOnLeftHandle{
    _tintColorLineOnLeftHandle = tintColorLineOnLeftHandle;
    self.sliderLineOnLeftHandle.backgroundColor = [tintColorLineOnLeftHandle CGColor];
}
